...
df2=web.get_data_yahoo(symbol2,start,end)

#caculate the percentage of return
pct_change=df['Adj Close'].pct_change().fillna(0).cumsum()
pct_change2=df2['Adj Close'].pct_change().fillna(0).cumsum()

#pull example of top 10 rows
print(pct_change[:10])
print(pct_change2[:10])

#create fig
fig=plt.figure()

#setup the fig name
fig.canvas.set_window_title('Index Accumulated Return since ' +
...
ax1.plot(pct_change2, label='Down Jone Industries Average')
ax1.set_title('Accumulated Return')
